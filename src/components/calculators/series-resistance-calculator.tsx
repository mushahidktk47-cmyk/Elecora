...
import { numericResistorField } from "@/lib/validation/series-resistance";
import { calculateSeriesResistance } from "@/calculations/series-resistance";
...
/**
 * The first calculator with a dynamic list of inputs rather than a
 * fixed set of fields. Each row gets a stable numeric id (not its
 * array position), since resistors can be removed from the middle of
 * the list — using position as a React key would cause React to mix
 * up which input owns which state after a removal.
 */
export function SeriesResistanceCalculator() {
  const baseId = useId();
  ...
  return (
    <div className="flex flex-col gap-6">
      <FormulaDisplay label="Series Resistance" formula="Rtotal = R1 + R2 + ... + Rn" />
      ...
      {!allFilled ? (
        <p className="text-sm text-muted-foreground">
          Enter a value for every resistor to calculate the total.
        </p>
